...
async function fetchLyrics(track) {
    ...
    try {
        ...
        const res = await fetch(`https://lrclib.net/api/get?${params}`, { headers: LRC_HEADERS, signal: ctrl });
        if (res.ok) data = await res.json();
        else {
            const s = await fetch(`https://lrclib.net/api/search?${new URLSearchParams({ track_name: track.info.title ?? "", artist_name: track.info.author ?? "" })}`, { headers: LRC_HEADERS, signal: AbortSignal.timeout(10000) });
