...
async function autoPlayFunction(player, lastPlayedTrack) {
    ...
    if (src === "youtube" || src === "youtubemusic") {
        ...
        // Spotify / others: text search on the same artist + title.
        // (Keeps working regardless of which Lavalink search plugins are installed.)
        res = await player.search({
            query: `${info.author ?? ""} - ${info.title}`.trim(),
            source: "ytmsearch"
        }, requester).catch(e => {
            global.log.warn(`[${gid}] autoplay: related search failed — ${e?.message ?? e}`);
            return null;
        });
    }
