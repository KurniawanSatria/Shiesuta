...
function nowPlayingCard(guildId, track, requester, lines, pos) {
    ...
    const comps = [
        ...
        },
        {
            ...
            components: [{
                type: 10,
                content: `### ${info.sourceName === "spotify" ? EMOJI.spotify : info.sourceName === "soundcloud" ? EMOJI.soundcloud : info.sourceName === "youtube" ? EMOJI.youtube : EMOJI.music} ${t.nowPlaying}\n**[${info.title}](${info.uri ?? ""})**\n${EMOJI.artist} \`${info.author ?? t.unknown}\`\n${EMOJI.time} \`${formatDuration(info.duration ?? info.length)}\`\n\n\n\n`
            }],
            accessory: {
                type: 11,
