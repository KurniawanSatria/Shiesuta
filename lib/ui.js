...
function nowPlayingCard(guildId, player, track, requester, lines, pos, lyricsVisible = false) {
    ...
    const repeatEmoji = repeatMode === "track" ? EMOJI.repeat_one : EMOJI.repeat;
    const repeatStyle = repeatMode === "off" ? 2 : 1;
    const autoplayOn = player.getData?.("autoplay_disabled") !== true;
    comps.push(
        { type: 14 },
        {
            type: 1, components: [
                ...
                ...(lines?.length ? [{ type: 2, custom_id: "lyrics_toggle", emoji: buttonEmoji(lyricsVisible ? EMOJI.lyrics_disabled : EMOJI.lyrics), style: 2 }] : [{ type: 2, custom_id: "np_queue", emoji: buttonEmoji(EMOJI.queue), style: 2 }])
            ]
        },
        {
            type: 1, components: [
                { type: 2, custom_id: "np_repeat", emoji: buttonEmoji(repeatEmoji), style: repeatStyle },
                { type: 2, custom_id: "np_vol_down", emoji: buttonEmoji(EMOJI.volume_low), style: 2 },
                { type: 2, custom_id: "np_autoplay", emoji: buttonEmoji(EMOJI.autoplay), style: autoplayOn ? 1 : 2 },
                { type: 2, custom_id: "np_vol_up", emoji: buttonEmoji(EMOJI.volume_medium), style: 2 },
                { type: 2, custom_id: "np_shuffle", emoji: buttonEmoji(EMOJI.shuffle), style: 2, disabled: (player.queue?.tracks?.length ?? 0) < 2 },
            ]
        });
