...
module.exports = {
    ...
    async run(ctx, player, track, payload) {
        global.log.info(`Queue ended on ${player.guildId} ${player.guild?.name ?? ""}`);
        ...
        let endedMsg = null;
        if (channel) endedMsg = await channel.send(card({ title: `## ${EMOJI.bye} ${T(player.guildId).queueEndedTitle}`, body: `${mentions ? `${mentions}\n` : ""}${T(player.guildId).queueEnded} <t:${Math.floor(Date.now() / 1000) + 60}:R>`, thumb: ctx.client.user.displayAvatarURL()
        })).catch(() => { });
