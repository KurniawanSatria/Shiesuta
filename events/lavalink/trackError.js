...
module.exports = {
    ...
    async run(ctx, player, track, payload) {
        ...
        const severity = payload?.exception?.severity ?? "COMMON";
        ...
        const otherNodes = [...ctx.lavalink.nodeManager.nodes.values()].filter(n => n.connected && n.sessionId && n.id !== player.node?.id);
        if (["FAULT", "SUSPICIOUS"].includes(severity) && trackKey && otherNodes.length && player.getData("trackErrorFailover") !== trackKey) {
            player.setData("trackErrorFailover", trackKey);
            try {
                const oldNode = player.node?.id;
                const targetNode = otherNodes.sort((a, b) => (a.stats?.playingPlayers ?? 0) - (b.stats?.playingPlayers ?? 0))[0];
                const newNode = await player.moveNode(targetNode.id);
                global.log.warn(`Track error (${severity}) on ${player.guildId}: moved player from ${oldNode} to ${targetNode.id}`);
                return newNode;
            } catch (error) {
