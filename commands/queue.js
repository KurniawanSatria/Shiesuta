...
module.exports = {
    ...
    async run(m, args, { player, t }) {
        ...
        if (!upcoming.length && !cur) return m.reply(reply(`### ${EMOJI.queue} ${t.queueTitle}`, t.queueEmpty));
        const per = 10;
        ...
        if (pages > 1) body.push("", `-# ${EMOJI.queue} ${(t.page ?? "Page {p}/{n}").replace("{p}", page).replace("{n}", pages)}`);
        return m.reply(card({ title: `### ${EMOJI.queue} ${t.queueTitle}`, body: body.join("\n"), thumb: cur?.info?.artworkUrl }));
    }
};
