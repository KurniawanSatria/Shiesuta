...
module.exports = {
    ...
    async run(m, args, { player, t }) {
        ...
        if (!args.length) return m.reply(reply(`### ${EMOJI.volume} Volume`, t.volCurrent.replace("{vol}", current)));
        ...
        return m.reply(reply(`### ${EMOJI.volume} Volume`, t.volSet.replace("{vol}", val)));
    }
};
